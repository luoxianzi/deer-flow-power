...
from .backend import SandboxBackend, probe_sandbox_http, wait_for_sandbox_ready
from .local_backend import LocalContainerBackend
from .tool_runtime_diagnostics import record_recovery, set_provider_class
from .tool_runtime_env import (
    tool_autorecover_enabled,
    tool_debug_enabled,
    tool_healthcheck_enabled,
    tool_probe_timeout_seconds,
    tool_recover_backoff_base_seconds,
    tool_recover_max_attempts,
)
...
class AioSandboxProvider(SandboxProvider):
    ...
    def __init__(self):
        ...
        set_provider_class(self.__class__.__name__)
    ...
    # ── Tool runtime health / recovery (HTTP bridge) ─────────────────────

    def ping_sandbox_http(self, sandbox_id: str) -> bool:
        """Return True if the sandbox HTTP API responds OK for this in-process handle."""
        with self._lock:
            sandbox = self._sandboxes.get(sandbox_id)
            url = sandbox.base_url if sandbox else None
        if not url:
            return False
        timeout = tool_probe_timeout_seconds()
        ok = probe_sandbox_http(url, timeout=timeout)
        if tool_debug_enabled():
            logger.info("sandbox_http_probe sandbox_id=%s url=%s ok=%s", sandbox_id, url, ok)
        return ok

    def tool_diagnostics_snapshot(self) -> dict:
        """Merge provider-local state with global tool-runtime diagnostics."""
        from .tool_runtime_diagnostics import snapshot

        out = snapshot()
        with self._lock:
            out["active_sandbox_ids"] = sorted(self._sandboxes.keys())
            out["warm_pool_sandbox_ids"] = sorted(self._warm_pool.keys())
            out["thread_to_sandbox"] = dict(self._thread_sandboxes)
            out["healthcheck_enabled"] = tool_healthcheck_enabled()
            out["autorecover_enabled"] = tool_autorecover_enabled()
        return out

    def invalidate_sandbox_bridge(self, sandbox_id: str, *, reason: str) -> None:
        """Tear down a sandbox that failed HTTP reachability (or similar)."""
        record_recovery(f"invalidate:{reason}", sandbox_id=sandbox_id)
        logger.warning("Invalidating sandbox bridge %s (%s)", sandbox_id, reason)
        try:
            self.destroy(sandbox_id)
        except Exception as e:
            logger.error("Failed to destroy sandbox %s during invalidation: %s", sandbox_id, e)

    def recover_sandbox_for_thread(self, thread_id: str, dead_sandbox_id: str) -> str:
        """Destroy dead sandbox and acquire a fresh one for thread_id (same deterministic id policy)."""
        self.invalidate_sandbox_bridge(dead_sandbox_id, reason="recover_for_thread")
        # Exponential backoff: brief sleep between destroy and recreate to avoid port races
        delay = tool_recover_backoff_base_seconds()
        for attempt in range(1, tool_recover_max_attempts() + 1):
            if attempt > 1:
                time.sleep(delay * (2 ** (attempt - 2)))
            try:
                new_id = self._acquire_internal(thread_id)
                record_recovery(f"reacquired attempt={attempt}", sandbox_id=new_id)
                return new_id
            except Exception as e:
                logger.warning("recover_sandbox_for_thread attempt %s failed: %s", attempt, e)
                if attempt == tool_recover_max_attempts():
                    raise
        raise RuntimeError("recover_sandbox_for_thread: exhausted retries")
    ...
    def _acquire_internal(self, thread_id: str | None) -> str:
        ...
        if thread_id:
            ...
            with self._lock:
                if thread_id in self._thread_sandboxes:
                    ...
                    elif not tool_healthcheck_enabled():
                        logger.info(f"Reusing in-process sandbox {existing_id} for thread {thread_id}")
                        self._last_activity[existing_id] = time.time()
                        return existing_id
                    else:
                        cached_id = existing_id
                        cached_url = self._sandboxes[existing_id].base_url
            if cached_id is not None and cached_url is not None:
                if probe_sandbox_http(cached_url, timeout=tool_probe_timeout_seconds()):
                    with self._lock:
                        if (
                            thread_id in self._thread_sandboxes
                            and self._thread_sandboxes[thread_id] == cached_id
                            and cached_id in self._sandboxes
                        ):
                            logger.info(f"Reusing in-process sandbox {cached_id} for thread {thread_id}")
                            self._last_activity[cached_id] = time.time()
                            return cached_id
                else:
                    logger.warning("In-process sandbox %s failed HTTP health check; tearing down", cached_id)
                    self.invalidate_sandbox_bridge(cached_id, reason="cache_reuse_probe_failed")
        ...
        if thread_id:
            ...
            if warm_info is not None:
                if tool_healthcheck_enabled() and not probe_sandbox_http(
                    warm_info.sandbox_url, timeout=tool_probe_timeout_seconds()
                ):
                    logger.warning(
                        "Warm-pool sandbox %s failed HTTP health check; destroying stale container",
                        sandbox_id,
                    )
                    try:
                        self._backend.destroy(warm_info)
                    except Exception as e:
                        logger.error("Failed to destroy dead warm-pool sandbox %s: %s", sandbox_id, e)
                else:
                    sandbox = AioSandbox(id=sandbox_id, base_url=warm_info.sandbox_url)
                    with self._lock:
                        self._sandboxes[sandbox_id] = sandbox
                        self._sandbox_infos[sandbox_id] = warm_info
                        self._last_activity[sandbox_id] = time.time()
                        self._thread_sandboxes[thread_id] = sandbox_id
                    logger.info(
                        f"Reclaimed warm-pool sandbox {sandbox_id} for thread {thread_id} at {warm_info.sandbox_url}"
                    )
                    return sandbox_id
    ...
    def _discover_or_create_with_lock(self, thread_id: str, sandbox_id: str) -> str:
        ...
        with open(lock_path, "a", encoding="utf-8") as lock_file:
            ...
            try:
                ...
                post_lock_id: str | None = None
                post_lock_url: str | None = None
                with self._lock:
                    if thread_id in self._thread_sandboxes:
                        existing_id = self._thread_sandboxes[thread_id]
                        if existing_id not in self._sandboxes:
                            ...
                            logger.info(
                                f"Reusing in-process sandbox {existing_id} for thread {thread_id} (post-lock check)"
                            )
                            self._last_activity[existing_id] = time.time()
                            return existing_id
                        else:
                            post_lock_id = existing_id
                            post_lock_url = self._sandboxes[existing_id].base_url
                if post_lock_id is not None and post_lock_url is not None:
                    if probe_sandbox_http(post_lock_url, timeout=tool_probe_timeout_seconds()):
                        with self._lock:
                            if (
                                thread_id in self._thread_sandboxes
                                and self._thread_sandboxes[thread_id] == post_lock_id
                                and post_lock_id in self._sandboxes
                            ):
                                logger.info(
                                    f"Reusing in-process sandbox {post_lock_id} for thread {thread_id} (post-lock check)"
                                )
                                self._last_activity[post_lock_id] = time.time()
                                return post_lock_id
                    else:
                        logger.warning(
                            "In-process sandbox %s failed HTTP health check (post-lock); tearing down",
                            post_lock_id,
                        )
                        self.invalidate_sandbox_bridge(post_lock_id, reason="post_lock_cache_probe_failed")
                with self._lock:
                    if sandbox_id in self._warm_pool:
                        warm_info, _ = self._warm_pool.pop(sandbox_id)
                        if tool_healthcheck_enabled() and not probe_sandbox_http(
                            ...
                            logger.warning(
                                "Warm-pool sandbox %s failed HTTP health check (post-lock); destroying",
                                sandbox_id,
                            )
                            try:
                                self._backend.destroy(warm_info)
                            ...
                            self._thread_sandboxes[thread_id] = sandbox_id
                            logger.info(
                                f"Reclaimed warm-pool sandbox {sandbox_id} for thread {thread_id} (post-lock check)"
                            )
                            return sandbox_id
                ...
                if discovered is not None:
                    if tool_healthcheck_enabled() and not probe_sandbox_http(
                        discovered.sandbox_url, timeout=tool_probe_timeout_seconds()
                    ):
                        logger.warning(
                            "Discovered sandbox %s at %s failed HTTP health check; destroying stale container",
                            discovered.sandbox_id,
                            discovered.sandbox_url,
                        )
                        try:
                            self._backend.destroy(discovered)
                        except Exception as e:
                            logger.error("Failed to destroy unreachable discovered sandbox %s: %s", sandbox_id, e)
                    else:
                        ...
                        logger.info(
                            f"Discovered existing sandbox {discovered.sandbox_id} for thread {thread_id} at {discovered.sandbox_url}"
                        )
