...
def probe_sandbox_http(sandbox_url: str, *, timeout: float = 2.5) -> bool:
    """Single-shot HTTP probe: sandbox API is reachable and returns 200.

    Uses the same endpoint as readiness polling so behavior stays consistent.
    """
    try:
        response = requests.get(f"{sandbox_url.rstrip('/')}/v1/sandbox", timeout=timeout)
...
def wait_for_sandbox_ready(sandbox_url: str, timeout: int = 30) -> bool:
    ...
    while time.time() - start_time < timeout:
        if probe_sandbox_http(sandbox_url, timeout=5.0):
            return True
        time.sleep(1)
    return False
