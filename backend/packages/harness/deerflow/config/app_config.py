...
from deerflow.config.runtime_profile_config import RuntimeProfileConfig, default_runtime_profiles
...
class AppConfig(BaseModel):
    ...
    runtime_profiles: dict[str, RuntimeProfileConfig] = Field(
        default_factory=default_runtime_profiles,
        description="Named runtime profiles (flash/thinking/pro/ultra) that shape assistant execution defaults",
    )
    ...
    @classmethod
    def _merge_runtime_profiles_with_defaults(cls, value: Any) -> Any:
        """Merge user-defined runtime profiles on top of the built-in defaults."""
        if value is None or not isinstance(value, dict):
            return value

        merged: dict[str, Any] = default_runtime_profiles()
        for name, profile in value.items():
            normalized_name = name.strip().lower() if isinstance(name, str) else name
            base_profile = merged.get(normalized_name)
            if isinstance(profile, RuntimeProfileConfig):
                profile_update = profile.model_dump(exclude_none=True)
            elif isinstance(profile, dict):
                profile_update = {k: v for k, v in profile.items() if v is not None}
            else:
                merged[normalized_name] = profile
                continue

            if isinstance(base_profile, RuntimeProfileConfig):
                merged[normalized_name] = base_profile.model_copy(update=profile_update)
            else:
                merged[normalized_name] = RuntimeProfileConfig(**profile_update)
        return merged
