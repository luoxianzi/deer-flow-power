...
# Model substrings that signal the Opus 4.7 API contract (adaptive thinking only,
# no sampling params). New generations can be added here without touching any
# call sites. See https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7
_ADAPTIVE_THINKING_MODEL_MARKERS = ("opus-4-7",)

# Sampling parameters that Claude Opus 4.7+ rejects with HTTP 400 when set to a
# non-default value. Strip them in _get_request_payload for those models.
_OPUS_47_FORBIDDEN_SAMPLING_PARAMS = ("temperature", "top_p", "top_k")

# Beta header required to pass `output_config` (effort + task_budget) to the
# Messages API. Without it, Opus 4.7 silently ignores output_config.
_TASK_BUDGETS_BETA = "task-budgets-2026-03-13"

# Valid Anthropic `output_config.effort` levels. Anything else from the frontend
# (e.g. "minimal" from the Codex pipeline) must be coerced or dropped.
_ANTHROPIC_EFFORT_LEVELS = ("low", "medium", "high", "xhigh")

# Frontend reasoning_effort values the Codex/API-pool pipeline uses that do not
# map to any Anthropic effort level. Coerce these to the nearest reasonable
# Anthropic equivalent so UI selections still round-trip sensibly.
_REASONING_EFFORT_TO_ANTHROPIC = {
    "none": None,  # disable effort entirely
    "minimal": "low",
    "low": "low",
    "medium": "medium",
    "high": "high",
...
class ClaudeAuthenticationUnavailableError(ValueError):
    """Raised when Claude top-level chat cannot obtain a usable credential."""
...
class ClaudeChatModel(ChatAnthropic):
    ...
    # Opus 4.7+ only supports adaptive thinking; the legacy extended-thinking
    # `{"type": "enabled", "budget_tokens": N}` payload now returns HTTP 400.
    # When True, ClaudeChatModel rewrites any such payload to
    # `{"type": "adaptive", "display": thinking_display}` before it leaves the
    # provider. Auto-set to True in model_post_init for opus-4-7 model IDs.
    use_adaptive_thinking: bool = False
    thinking_display: str = "summarized"  # "summarized" | "omitted" (API default)
    # Opus 4.7 rejects non-default temperature/top_p/top_k. Auto-set to True for
    # opus-4-7 so any stray sampling params get stripped.
    strip_sampling_params: bool = False
    # If set, provider emits `output_config.effort` on each request and adds the
    # `task-budgets-2026-03-13` beta header. Auto-enabled for opus-4-7 because
    # the `effort` knob is the model-specific replacement for extended-thinking
    # `budget_tokens`. Overridable to `None` to disable entirely.
    ...
    _enable_output_config: bool = PrivateAttr(default=False)
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        # Auto-infer Opus 4.7 compatibility flags from the model ID so ops don't
        # have to remember to toggle them in config.yaml.
        model_id = (self.model or "").lower()
        if any(marker in model_id for marker in _ADAPTIVE_THINKING_MODEL_MARKERS):
            self.use_adaptive_thinking = True
            self.strip_sampling_params = True
            # output_config + task-budgets beta are the model-native way to steer
            # Opus 4.7 effort. Turn on the request-time wiring by default.
            self._enable_output_config = True
        ...
        if not current_key or current_key in ("your-anthropic-api-key",):
            raise ClaudeAuthenticationUnavailableError(
                "Claude authentication unavailable: no valid Anthropic API key or non-expired Claude Code OAuth credential was found."
            )
        ...
        # Task-budgets beta header — required for Opus 4.7 `output_config`.
        # Append to any existing anthropic-beta value so OAuth betas stay intact.
        if self._enable_output_config:
            existing_headers = dict(self.default_headers or {})
            existing_beta = existing_headers.get("anthropic-beta", "")
            beta_tokens = [t.strip() for t in existing_beta.split(",") if t.strip()]
            if _TASK_BUDGETS_BETA not in beta_tokens:
                beta_tokens.append(_TASK_BUDGETS_BETA)
            existing_headers["anthropic-beta"] = ",".join(beta_tokens)
    ...
    def _get_request_payload(
        ...
        **kwargs: Any,
        ...
        # Model-contract sanitization must run before thinking budget so the
        # adaptive rewrite is the authoritative thinking shape.
        if self.strip_sampling_params:
            self._strip_forbidden_sampling_params(payload)
        if self.use_adaptive_thinking:
            self._convert_thinking_to_adaptive(payload)
        if self._enable_output_config:
            self._apply_output_config(payload)
    ...
    def _apply_output_config(self, payload: dict) -> None:
        """Inject Anthropic `output_config.effort` when the operator selected an effort level.

        No-op when the resolved effort is None — e.g. user deliberately picked
        "none" or no reasoning_effort was supplied. That allows the model to
        use its default adaptive pacing.
        """
        effort = self._resolve_anthropic_effort()
        if effort is None:
            return

        existing = payload.get("output_config")
        output_config: dict = dict(existing) if isinstance(existing, dict) else {}
        output_config["effort"] = effort
        payload["output_config"] = output_config

    def _resolve_anthropic_effort(self) -> str | None:
        """Map the configured reasoning_effort (frontend/config) to an Anthropic value.

        Returns None if no valid mapping — the request is sent without
        `output_config.effort`, which defers to the model's own adaptive pacing.
        """
        raw = (self.reasoning_effort or "").strip().lower()
        if not raw:
            return None
        if raw in _REASONING_EFFORT_TO_ANTHROPIC:
            return _REASONING_EFFORT_TO_ANTHROPIC[raw]
        if raw in _ANTHROPIC_EFFORT_LEVELS:
            return raw
        logger.debug("Unknown reasoning_effort=%r for Claude model; dropping", raw)
        return None

    @staticmethod
    def _strip_forbidden_sampling_params(payload: dict) -> None:
        """Remove temperature/top_p/top_k on Opus 4.7+ where they raise HTTP 400."""
        for key in _OPUS_47_FORBIDDEN_SAMPLING_PARAMS:
            payload.pop(key, None)

    def _convert_thinking_to_adaptive(self, payload: dict) -> None:
        """Rewrite legacy extended-thinking payload to the adaptive-thinking shape.

        Opus 4.7 removed the `{"type": "enabled", "budget_tokens": N}` payload.
        `{"type": "adaptive"}` is the only supported thinking-on mode. We also
        default `display` to the class-level `thinking_display` so thinking
        progress remains visible in streams unless operators opt out.
        """
        ...
        if ttype == "disabled":
            # Caller explicitly turned thinking off — leave it.
            return

        # Strip the removed budget_tokens and force adaptive shape.
        payload["thinking"] = {
            "type": "adaptive",
            "display": thinking.get("display") or self.thinking_display,
        }
    ...
    def _apply_thinking_budget(self, payload: dict) -> None:
        """Auto-allocate thinking budget (80% of max_tokens) for extended thinking.

        Only applies to the legacy `{"type": "enabled"}` shape. Adaptive-thinking
        models (Opus 4.7+) do not accept `budget_tokens` and would return HTTP 400.
        """
        thinking = payload.get("thinking")
    ...
    def _reload_oauth_credential_from_disk(self) -> bool:
        """Re-read ~/.claude/.credentials.json and swap in the fresh token, if any.

        Returns True when a new token was adopted. Used for 401-retry: Claude
        Desktop rotates the on-disk token independently of our in-memory model
        instance, so stale cached tokens can cause Authentication errors even
        after the sync script has refreshed the file.
        """
        ...
        try:
            ...
            logger.debug("credential reload failed", exc_info=True)
            return False

        if not cred or not cred.access_token or cred.access_token == self._oauth_access_token:
            return False
        ...
        try:
            ...
            self.anthropic_api_key = SecretStr(cred.access_token)
        except Exception:
            pass
        self._patch_client_oauth(self._client)
        self._patch_client_oauth(self._async_client)
        logger.info("Claude OAuth credential reloaded from disk (source: %s)", cred.source)
        return True
    ...
    def _generate(self, messages: list[BaseMessage], stop: list[str] | None = None, **kwargs: Any) -> Any:
        ...
        for attempt in range(1, self.retry_max_attempts + 1):
            try:
                ...
                last_error = e
                # The on-disk credentials may have been rotated by Claude Desktop.
                # Try reloading once per call before giving up.
                if not auth_reload_attempted and self._reload_oauth_credential_from_disk():
                    auth_reload_attempted = True
                    logger.warning("Claude auth failed; reloaded credentials from disk and retrying")
                    continue
                raise
