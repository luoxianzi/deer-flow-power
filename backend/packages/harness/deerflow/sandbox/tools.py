...
from deerflow.sandbox.tool_bridge_recovery import (
    ephemeral_local_sandbox_for_fallback,
    exception_suggests_connection_failure,
    is_aio_sandbox_provider,
    run_aio_read_file_with_recover,
    run_aio_shell_with_recover,
    run_with_bridge_retry,
...
def _reject_unsafe_ssh_auth_patterns(command: str) -> None:
    """Block inline-password SSH patterns that are brittle and leak secrets."""
    lowered = command.lower()

    if "sshpass" in lowered:
        raise PermissionError(
            "Unsafe SSH command rejected: do not use sshpass or inline passwords. "
            "Use key-based SSH via /root/.ssh and a non-interactive command such as "
            "`ssh -o BatchMode=yes -o ConnectTimeout=8 <host-alias> 'hostname'`."
        )

    if "paramiko" in lowered and re.search(r"password\s*=", command, flags=re.IGNORECASE):
        raise PermissionError(
            "Unsafe SSH command rejected: do not embed Paramiko passwords in commands. "
            "Use the bash tool with key-based SSH and an existing host alias from /root/.ssh/config."
        )


def _get_configured_mounts() -> list:
    """Get configured mounts from sandbox config.

    Returns a list of VolumeMountConfig objects. Cached after first load.
    """
    cached = getattr(_get_configured_mounts, "_cached", None)
    ...
    try:
        ...
        _get_configured_mounts._cached = mounts  # type: ignore[attr-defined]
        return mounts
    except Exception:
        return []


def _is_configured_mount_path(path: str) -> bool:
    """Check if a path is under any configured mount's container_path."""
    for mount in _get_configured_mounts():
        prefix = mount.container_path
        if path == prefix or path.startswith(f"{prefix}/"):
            return True
    return False


def _get_mount_for_path(path: str):
    """Get the VolumeMountConfig for a given virtual path, or None."""
    for mount in _get_configured_mounts():
        prefix = mount.container_path
        if path == prefix or path.startswith(f"{prefix}/"):
            return mount
...
def _resolve_configured_mount_path(path: str) -> str:
    """Resolve a virtual mount path to its host filesystem path.

    Raises:
        PermissionError: If no matching mount is found or path traversal detected.
    """
    _reject_path_traversal(path)
    mount = _get_mount_for_path(path)
    if mount is None:
        raise PermissionError(f"No mount configuration found for path: {path}")

    if path == mount.container_path:
        return mount.host_path

    relative = path[len(mount.container_path) :].lstrip("/")
    resolved = Path(mount.host_path).resolve() / relative
    # Ensure resolved path stays inside the mount root
    try:
        resolved.resolve().relative_to(Path(mount.host_path).resolve())
    except ValueError:
...
def validate_local_tool_path(path: str, thread_data: ThreadDataState | None, *, read_only: bool = False) -> None:
    ...
    # Configured mount paths — respect per-mount read_only setting
    mount = _get_mount_for_path(path)
    if mount is not None:
        if not read_only and mount.read_only:
            raise PermissionError(f"Write access to mount path is not allowed: {path}")
        return
    ...
    if sandbox_state is not None:
        ...
        if sandbox_id is not None:
            ...
            if sandbox is not None:
                ...
                if thread_id and is_aio_sandbox_provider(provider):
                    ...
                    if tre.tool_healthcheck_enabled():
                        ping = getattr(provider, "ping_sandbox_http", None)
                        if callable(ping) and not ping(sandbox_id):
                            trd.record_failure(
                                tool_name="ensure_sandbox_initialized",
                                message="HTTP probe failed before tool",
                                sandbox_id=sandbox_id,
                                sandbox_url=getattr(sandbox, "base_url", None),
                            )
                            if tre.tool_autorecover_enabled():
                                recover = getattr(provider, "recover_sandbox_for_thread", None)
                                if callable(recover):
                                    logger.warning(
                                        "Sandbox %s failed HTTP probe; recovering for thread %s",
                                        sandbox_id,
                                        thread_id,
                                    )
                                    new_id = recover(thread_id, sandbox_id)
                                    runtime.state["sandbox"] = {"sandbox_id": new_id}
                                    sandbox = provider.get(new_id)
                                    if sandbox is None:
                                        raise SandboxNotFoundError(
                                            "Sandbox not found after HTTP recovery",
                                            sandbox_id=new_id,
                                        )
                                    if runtime.context is not None:
                                    ...
                                    return sandbox
                            trd.mark_degraded("sandbox_http_unreachable")
                            raise SandboxNotFoundError(
                                "Tool sandbox HTTP endpoint unreachable (probe failed). "
                                "Try DEERFLOW_TOOL_AUTORECOVER=1 or `deerflow repair tools`.",
                                sandbox_id=sandbox_id,
                            )
                if runtime.context is not None:
...
def bash_tool(runtime: ToolRuntime[ContextT, ThreadState], description: str, command: str) -> str:
    ...
    """
    try:
        _reject_unsafe_ssh_auth_patterns(command)
        ...
        # AioSandbox path: use tool-bridge recovery helpers when available, with
        # optional local ephemeral fallback on apparent connection failure.
        sandbox_id = runtime.state.get("sandbox", {}).get("sandbox_id") if runtime.state else None
        thread_id = runtime.context.get("thread_id") if runtime.context else None
        provider = get_sandbox_provider()
        if sandbox_id and thread_id and is_aio_sandbox_provider(provider):
            out = run_aio_shell_with_recover(
                runtime=runtime,
                tool_name="bash",
                thread_id=thread_id,
                sandbox_id=sandbox_id,
                execute=lambda sb: sb.execute_command(command),
            )
            from deerflow.community.aio_sandbox import tool_runtime_env as tre

            if tre.tool_fallback_local_enabled() and tool_output_suggests_connection_failure(out):
                try:
                    aio_thread_data = get_thread_data(runtime)
                except Exception:
                    aio_thread_data = None
                if aio_thread_data is not None:
                    try:
                        fb = ephemeral_local_sandbox_for_fallback()
                        validate_local_bash_command_paths(command, aio_thread_data)
                        cmd2 = replace_virtual_paths_in_command(command, aio_thread_data)
                        return _truncate_bash_output(
                            mask_local_paths_in_output(fb.execute_command(cmd2), aio_thread_data),
                            max_chars,
                        )
                    except Exception:
                        pass
            return _truncate_bash_output(out, max_chars)
...
def read_file_tool(
    ...
    try:
        ...
        provider = get_sandbox_provider()
        if (
            not is_local_sandbox(runtime)
            and sandbox_id
            and thread_id
            and is_aio_sandbox_provider(provider)
        ):
            aio_path = requested_path
            content = run_aio_read_file_with_recover(
                runtime=runtime,
                tool_name="read_file",
                thread_id=thread_id,
                sandbox_id=sandbox_id,
                read_fn=lambda sb, p=aio_path: sb.read_file(p),
            )
            from deerflow.community.aio_sandbox import tool_runtime_env as tre

            if (
                tre.tool_fallback_local_enabled()
                ...
                try:
                    fb = ephemeral_local_sandbox_for_fallback()
                    p2 = requested_path
                    validate_local_tool_path(p2, thread_data, read_only=True)
                    if _is_skills_path(p2):
                        p2 = _resolve_skills_path(p2)
                    elif _is_acp_workspace_path(p2):
                        p2 = _resolve_acp_workspace_path(p2, _extract_thread_id_from_thread_data(thread_data))
                    elif not _is_custom_mount_path(p2):
                        p2 = _resolve_and_validate_user_data_path(p2, thread_data)
                    content = fb.read_file(p2)
                except Exception:
                    pass
        else:
...
def write_file_tool(
    ...
    try:
        ...
        if (
            ...
            wpath = requested_path
            try:
                with get_file_operation_lock(sandbox, wpath):
                    run_with_bridge_retry(
                        runtime=runtime,
                        tool_name="write_file",
                        thread_id=thread_id,
                        sandbox_id=sandbox_id,
                        fn=lambda sb, wp=wpath, c=content, a=append: sb.write_file(wp, c, a),
                    )
                return "OK"
                ...
                try:
                    fb = ephemeral_local_sandbox_for_fallback()
                    validate_local_tool_path(requested_path, thread_data)
                    if not _is_custom_mount_path(requested_path):
                        p2 = _resolve_and_validate_user_data_path(requested_path, thread_data)
                    else:
                        p2 = requested_path
                    with get_file_operation_lock(fb, p2):
                        fb.write_file(p2, content, append)
                    return "OK"
                except Exception:
                    raise exc
...
def str_replace_tool(
    ...
    try:
        ...
        # AioSandbox path: use tool-bridge recovery for both the read and write
        # legs, with an optional ephemeral local fallback on connection failure.
        if (
            not is_local_sandbox(runtime)
            and sandbox_id
            and thread_id
            ...
            aio_path = requested_path
            write_sandbox = sandbox
            write_path = aio_path
            used_host_fallback_read = False

            with get_file_operation_lock(sandbox, aio_path):
                content = run_aio_read_file_with_recover(
                    runtime=runtime,
                ...
                if (
                    ...
                    try:
                        fb = ephemeral_local_sandbox_for_fallback()
                        p2 = requested_path
                        validate_local_tool_path(p2, thread_data)
                        if not _is_custom_mount_path(p2):
                            p2 = _resolve_and_validate_user_data_path(p2, thread_data)
                        content = fb.read_file(p2)
                        write_sandbox = fb
                        write_path = p2
                        used_host_fallback_read = True
                    except Exception:
                        pass
                ...
                if not used_host_fallback_read:
                    try:
                        run_with_bridge_retry(
                            runtime=runtime,
                            tool_name="str_replace_write",
                            thread_id=thread_id,
                            sandbox_id=sandbox_id,
                            fn=lambda sb, wp=write_path, c=content: sb.write_file(wp, c),
                        )
                    except BaseException as exc:
                        if not (
                            tre.tool_fallback_local_enabled()
                        ...
                        try:
                            ...
                            if not _is_custom_mount_path(requested_path):
                                ...
                                p2 = requested_path
                            fb.write_file(p2, content)
                        except Exception:
                            raise exc
                else:
                    write_sandbox.write_file(write_path, content)
            return "OK"

        # Default path: local sandbox (or other provider without bridge recovery).
        with get_file_operation_lock(sandbox, path):
            content = sandbox.read_file(path)
            if not content:
                return "OK"
