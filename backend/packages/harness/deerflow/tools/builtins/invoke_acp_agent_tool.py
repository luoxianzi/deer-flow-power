...
DEFAULT_ACP_STDIO_LIMIT = 1024 * 1024
...
def _format_invocation_error(agent: str, cmd: str, exc: Exception) -> str:
    ...
    if "hit your limit" in lowered or "quota" in lowered or "rate limit" in lowered:
        return (
            f"ACP agent '{agent}' is temporarily unavailable because the upstream provider quota was exhausted. "
            f"Upstream error: {error_text}. "
            "Do not retry this ACP agent in the current run; fall back to DeerFlow built-in tools or task subagents and retry after the provider resets."
        )

    if not isinstance(exc, FileNotFoundError):
        return f"Error invoking ACP agent '{agent}': {error_text}"
...
def _get_acp_stdio_limit() -> int:
    """Return the stdio reader limit used for ACP subprocesses.

    Claude ACP sessions can emit large single-line JSON messages that exceed the
    default asyncio subprocess reader limit and crash the receive loop.
    """
    raw_value = os.getenv("DEER_FLOW_ACP_STDIO_LIMIT")
    if not raw_value:
        return DEFAULT_ACP_STDIO_LIMIT

    try:
        parsed = int(raw_value)
    except ValueError:
        logger.warning(
            "Invalid DEER_FLOW_ACP_STDIO_LIMIT=%r; falling back to %d",
            raw_value,
    ...
    if parsed < 65536:
        logger.warning(
            "DEER_FLOW_ACP_STDIO_LIMIT=%d is too small; falling back to %d",
            parsed,
            DEFAULT_ACP_STDIO_LIMIT,
        )
        return DEFAULT_ACP_STDIO_LIMIT
...
def build_invoke_acp_agent_tool(agents: dict) -> BaseTool:
    ...
    async def _invoke(agent: str, prompt: str, config: Annotated[RunnableConfig, InjectedToolArg] = None) -> str:
        ...
        try:
            ...
            stdio_limit = _get_acp_stdio_limit()
            async with spawn_agent_process(
                ...
                *args,
                ...
                logger.info(
                    "Spawning ACP agent '%s' with command '%s' and args %s in cwd %s (stdio limit: %d)",
                    agent,
                    cmd,
                    args,
                    physical_cwd,
