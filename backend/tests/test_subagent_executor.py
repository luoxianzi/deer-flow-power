...
class TestAsyncExecutionPath:
    ...
    @pytest.mark.anyio
    async def test_aexecute_retries_custom_agent_after_recursion_limit(self, classes, mock_agent, msg):
        """Custom worker agents should progressively retry with larger recursion budgets."""
        SubagentExecutor = classes["SubagentExecutor"]
        ...
        final_state = {
            "messages": [
                msg.human("Task"),
                msg.ai("Recovered after retry", "msg-retry"),
            ]
        }
        seen_limits: list[int] = []

        async def mock_astream(*args, **kwargs):
            recursion_limit = kwargs["config"]["recursion_limit"]
            seen_limits.append(recursion_limit)
            if recursion_limit == 80:
        ...
        assert result.status == SubagentStatus.COMPLETED
        assert result.result == "Recovered after retry"
        assert seen_limits[0] == 80
        assert seen_limits[1] == 120
        assert len(seen_limits) == 2

    @pytest.mark.anyio
    async def test_aexecute_salvages_partial_result_after_all_budgets_exhausted(self, classes, mock_agent, msg):
        """When all budgets are exhausted, partial AI messages should be returned instead of a raw error."""
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentConfig = classes["SubagentConfig"]
        SubagentStatus = classes["SubagentStatus"]
        ...
        partial_state = {
            "messages": [
                msg.human("Task"),
                msg.ai("Partial findings so far...", "msg-partial"),
            ]
        }
        seen_limits: list[int] = []
        ...
        config = SubagentConfig(
            name="heavy-audit-agent",
            description="Heavy task worker",
            system_prompt="prompt",
            max_turns=80,
            timeout_seconds=60,
            source="custom",
            agent_name="heavy-audit-agent",
            thinking_enabled=None,
        )
        executor = SubagentExecutor(
            config=config,
        ...
        assert result.status == SubagentStatus.COMPLETED
        assert "Partial findings so far..." in result.result
        assert "部分結果" in result.result
        assert len(seen_limits) == 3

    @pytest.mark.anyio
    async def test_aexecute_fails_gracefully_when_no_partial_result(self, classes, mock_agent, msg):
        """When all budgets exhausted and no AI messages collected, return a clear error."""
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentConfig = classes["SubagentConfig"]
        SubagentStatus = classes["SubagentStatus"]
        ...
        assert "exhausted all recursion budgets" in result.error
    ...
    @pytest.mark.anyio
    async def test_aexecute_treats_llm_error_message_as_failed(self, classes, base_config, mock_agent):
        """Test that middleware-generated LLM error messages do not count as successful subagent output."""
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentStatus = classes["SubagentStatus"]
        ...
        final_message = AIMessage(
            content="当前模型供应商暂时不可用，且已禁用自动切换到本机后备模型。请稍后重试。",
            additional_kwargs={"llm_error": True, "llm_error_reason": "transient"},
        )
        final_state = {
            "messages": [
        ...
        assert result.status == SubagentStatus.FAILED
        assert "当前模型供应商暂时不可用" in result.error
        assert result.completed_at is not None
...
class TestSubagentRuntimeResolution:
    def test_custom_agent_inherits_parent_reasoning(self, classes):
        ...
        model_name, thinking_enabled, reasoning_effort = executor_module._resolve_model_runtime(
            config,
            parent_model="api-pool-default",
            parent_thinking_enabled=True,
            parent_reasoning_effort="high",
        )

        assert model_name == "api-pool-default"
        assert thinking_enabled is True
        assert reasoning_effort == "high"

    def test_builtin_subagent_keeps_non_thinking_default(self, classes):
        import deerflow.subagents.executor as executor_module
...
class TestBuildTurnBudgets:
    """Verify progressive retry budget calculation."""

    def test_builtin_agent_gets_single_budget(self):
        import deerflow.subagents.executor as executor_module
        budgets = executor_module._build_turn_budgets(80, "builtin")
        assert budgets == [80]

    def test_custom_agent_gets_progressive_budgets(self):
        import deerflow.subagents.executor as executor_module
        budgets = executor_module._build_turn_budgets(80, "custom")
        assert budgets == [80, 120, 160]

    def test_custom_agent_budgets_capped_by_ceiling(self, monkeypatch):
        import deerflow.subagents.executor as executor_module
        monkeypatch.setenv("DEER_FLOW_SUBAGENT_RECURSION_CEILING", "100")
        budgets = executor_module._build_turn_budgets(80, "custom")
        assert budgets == [80, 100]
        assert all(b <= 100 for b in budgets)

    def test_high_max_turns_respected(self):
        import deerflow.subagents.executor as executor_module
        budgets = executor_module._build_turn_budgets(150, "custom")
        assert budgets[0] == 150
        assert budgets[-1] <= 200

    def test_env_ceiling_overrides_default(self, monkeypatch):
        import deerflow.subagents.executor as executor_module
        monkeypatch.setenv("DEER_FLOW_SUBAGENT_RECURSION_CEILING", "300")
        budgets = executor_module._build_turn_budgets(100, "custom")
