...
class TestAgentClassification:
    def test_worker_agents_are_delegatable(self):
        from deerflow.config.agents_config import AgentConfig, classify_agent_config, is_agent_delegation_target

        cfg = AgentConfig(name="data-architect-agent", tags=["data", "schema"])

        assert classify_agent_config(cfg) == "worker"
        assert is_agent_delegation_target(cfg) is True

    def test_orchestrator_agents_are_not_delegatable(self):
        from deerflow.config.agents_config import AgentConfig, classify_agent_config, is_agent_delegation_target

        cfg = AgentConfig(name="global-chief-architect", tags=["primary", "orchestration"])

        assert classify_agent_config(cfg) == "orchestrator"
        assert is_agent_delegation_target(cfg) is False

    def test_legacy_agents_are_not_delegatable(self):
        from deerflow.config.agents_config import AgentConfig, classify_agent_config, is_agent_delegation_target

        cfg = AgentConfig(name="omniai-network-guardian", tags=["legacy", "frontend-only"])

        assert classify_agent_config(cfg) == "legacy"
        assert is_agent_delegation_target(cfg) is False

    def test_list_delegatable_custom_agents_filters_manual_only_classes(self, tmp_path):
        _write_agent(tmp_path, "worker-agent", {"name": "worker-agent", "tags": ["qa"]})
        _write_agent(tmp_path, "chief-agent", {"name": "chief-agent", "tags": ["primary", "orchestration"]})
        _write_agent(tmp_path, "legacy-agent", {"name": "legacy-agent", "tags": ["legacy"]})

        with patch("deerflow.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from deerflow.config.agents_config import list_delegatable_custom_agents

            agents = list_delegatable_custom_agents()

        assert [agent.name for agent in agents] == ["worker-agent"]
