...
def test_load_claude_code_credential_ignores_directory_path(tmp_path, monkeypatch):
    _clear_claude_code_env(monkeypatch)
    # Redirect HOME into an empty sandbox dir so the default `~/.claude/.credentials.json`
    # fallback cannot pick up the developer's real credentials file while asserting
    # the directory-path-rejection behavior.
