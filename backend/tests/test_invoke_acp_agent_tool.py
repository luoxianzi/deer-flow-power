...
from deerflow.tools.builtins.invoke_acp_agent_tool import (
    DEFAULT_ACP_STDIO_LIMIT,
    _build_acp_mcp_servers,
    _build_mcp_servers,
    _get_acp_stdio_limit,
...
def test_get_acp_stdio_limit_defaults_to_safe_large_limit(monkeypatch):
    monkeypatch.delenv("DEER_FLOW_ACP_STDIO_LIMIT", raising=False)
    assert _get_acp_stdio_limit() == DEFAULT_ACP_STDIO_LIMIT


def test_get_acp_stdio_limit_rejects_invalid_values(monkeypatch):
    monkeypatch.setenv("DEER_FLOW_ACP_STDIO_LIMIT", "tiny")
    assert _get_acp_stdio_limit() == DEFAULT_ACP_STDIO_LIMIT

    monkeypatch.setenv("DEER_FLOW_ACP_STDIO_LIMIT", "1024")
    assert _get_acp_stdio_limit() == DEFAULT_ACP_STDIO_LIMIT


def test_format_invocation_error_detects_upstream_quota_exhaustion():
    message = _format_invocation_error(
        "claude_code_sonnet",
        "npx",
        RuntimeError("Internal error: You've hit your limit · resets 9am (UTC)"),
    )

    assert "temporarily unavailable" in message
    assert "quota was exhausted" in message
    assert "Do not retry this ACP agent in the current run" in message
...
async def test_invoke_acp_agent_uses_fixed_acp_workspace(monkeypatch, tmp_path):
    ...
    class DummyProcessContext:
        def __init__(self, client, cmd, *args, cwd, transport_kwargs=None):
    ...
    monkeypatch.setitem(
        ...
        SimpleNamespace(
            ...
            spawn_agent_process=lambda client, cmd, *args, env=None, cwd, transport_kwargs=None: DummyProcessContext(
                client,
                cmd,
                *args,
                cwd=cwd,
    ...
    assert captured["spawn"] == {
        ...
        "args": ["--json"],
        "cwd": expected_cwd,
        "transport_kwargs": {"limit": DEFAULT_ACP_STDIO_LIMIT},
    }
...
async def test_invoke_acp_agent_uses_per_thread_workspace_when_thread_id_in_config(monkeypatch, tmp_path):
    ...
    class DummyProcessContext:
        def __init__(self, client, cmd, *args, cwd, transport_kwargs=None):
            captured["cwd"] = cwd
            captured["transport_kwargs"] = transport_kwargs
    ...
    assert captured["transport_kwargs"] == {"limit": DEFAULT_ACP_STDIO_LIMIT}
...
async def test_invoke_acp_agent_passes_env_to_spawn(monkeypatch, tmp_path):
    ...
    class DummyProcessContext:
        def __init__(self, client, cmd, *args, env=None, cwd, transport_kwargs=None):
            captured["env"] = env
...
async def test_invoke_acp_agent_skips_invalid_mcp_servers(monkeypatch, tmp_path, caplog):
    ...
    class DummyProcessContext:
        def __init__(self, client, cmd, *args, env=None, cwd=None, transport_kwargs=None):
