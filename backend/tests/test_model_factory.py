...
from deerflow.models import standard_api_provider as standard_api_provider_module
...
def test_claude_auth_failure_does_not_fall_back_by_default(monkeypatch):
    cfg = _make_app_config(
        [
            _make_model("claude", use="deerflow.models.claude_provider:ClaudeChatModel"),
            _make_model("gpt", use="langchain_openai:ChatOpenAI"),
        ]
    )
    monkeypatch.setattr(factory_module, "get_app_config", lambda: cfg)
    ...
    class ExplodingClaudeModel(FakeChatModel):
        def __init__(self, **kwargs):
            raise ClaudeAuthenticationUnavailableError("expired oauth")
    ...
    def resolve(path, _base):
        if path == "deerflow.models.claude_provider:ClaudeChatModel":
            return ExplodingClaudeModel
        return CapturingModel

    monkeypatch.setattr(factory_module, "resolve_class", resolve)

    with pytest.raises(ClaudeAuthenticationUnavailableError, match="expired oauth"):
        factory_module.create_chat_model(name="claude")

    assert captured == {}


def test_claude_auth_failure_can_fall_back_when_explicitly_enabled(monkeypatch):
    cfg = _make_app_config(
        [
            _make_model("claude", use="deerflow.models.claude_provider:ClaudeChatModel"),
            _make_model("gpt", use="langchain_openai:ChatOpenAI"),
    ...
    factory_module.create_chat_model(name="claude", _allow_auth_fallback=True)

    assert captured.get("model") == "gpt"


def test_wrapped_claude_auth_failure_still_falls_back_when_explicitly_enabled(monkeypatch):
    cfg = _make_app_config(
        [
            _make_model("claude", use="deerflow.models.claude_provider:ClaudeChatModel"),
            _make_model("gpt", use="langchain_openai:ChatOpenAI"),
    ...
    class WrappedClaudeErrorModel(FakeChatModel):
        def __init__(self, **kwargs):
            raise ValueError(
                "1 validation error for ClaudeChatModel\n"
                "  Value error, Claude authentication unavailable: expired oauth"
            )
    ...
    def resolve(path, _base):
        if path == "deerflow.models.claude_provider:ClaudeChatModel":
            return WrappedClaudeErrorModel
        return CapturingModel
...
def test_claude_auth_failure_raises_when_no_fallback_model_exists(monkeypatch):
    cfg = _make_app_config([_make_model("claude", use="deerflow.models.claude_provider:ClaudeChatModel")])
    monkeypatch.setattr(factory_module, "get_app_config", lambda: cfg)
    monkeypatch.setattr(factory_module, "is_tracing_enabled", lambda: False)
    ...
    monkeypatch.setattr(factory_module, "resolve_class", lambda path, base: ExplodingClaudeModel)
...
class FakeStandardAPIChatModel(FakeChatModel):
    pass
...
def test_api_pool_provider_preserves_explicit_reasoning_effort_for_gpt5_runtime(monkeypatch):
    cfg = _make_app_config(
        [
            _make_model(
                "api-pool-default",
    ...
    _patch_factory(monkeypatch, cfg, model_class=FakeStandardAPIChatModel)
    monkeypatch.setattr(standard_api_provider_module, "StandardAPIChatModel", FakeStandardAPIChatModel)
    monkeypatch.setenv("API_POOL_MODEL", "gpt-5.4")

    FakeChatModel.captured_kwargs = {}
    factory_module.create_chat_model(name="api-pool-default", thinking_enabled=True, reasoning_effort="high")

    assert FakeChatModel.captured_kwargs.get("reasoning_effort") == "high"


def test_api_pool_provider_defaults_to_minimal_reasoning_when_thinking_disabled(monkeypatch):
    cfg = _make_app_config(
        [
            _make_model(
                "api-pool-default",
    ...
    FakeChatModel.captured_kwargs = {}
    factory_module.create_chat_model(name="api-pool-default", thinking_enabled=False)

    assert FakeChatModel.captured_kwargs.get("reasoning_effort") == "none"


def test_api_pool_provider_normalizes_explicit_minimal_reasoning_to_none(monkeypatch):
    cfg = _make_app_config(
        [
            _make_model(
                "api-pool-default",
    ...
    FakeChatModel.captured_kwargs = {}
    factory_module.create_chat_model(name="api-pool-default", thinking_enabled=True, reasoning_effort="minimal")
