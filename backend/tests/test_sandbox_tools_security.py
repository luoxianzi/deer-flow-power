...
from deerflow.sandbox.tools import (
    ...
    _is_skills_path,
    _reject_unsafe_ssh_auth_patterns,
...
def test_reject_unsafe_ssh_auth_patterns_blocks_sshpass() -> None:
    with pytest.raises(PermissionError, match="sshpass"):
        _reject_unsafe_ssh_auth_patterns("sshpass -p 'secret' ssh root@example.com")


def test_reject_unsafe_ssh_auth_patterns_blocks_paramiko_password() -> None:
    with pytest.raises(PermissionError, match="Paramiko"):
        _reject_unsafe_ssh_auth_patterns(
            "python - <<'PY'\nimport paramiko\nclient.connect(hostname='example.com', username='root', password='secret')\nPY"
        )


def test_reject_unsafe_ssh_auth_patterns_allows_key_based_ssh() -> None:
    _reject_unsafe_ssh_auth_patterns("ssh -o BatchMode=yes -o ConnectTimeout=8 omniai-sg 'hostname'")
