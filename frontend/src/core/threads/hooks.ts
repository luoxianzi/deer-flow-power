...
import type { AgentEngine, AgentThread, AgentThreadState } from "./types";
...
export function useThreadStream({
  ...
  const { models } = useModels({ enabled: !isMock });
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        const modelEntry =
          context.model_name != null && context.model_name !== ""
            ? models.find((m) => m.name === context.model_name)
            : undefined;
        // Align engine with the selected model's provider family. Stale localStorage
        // (e.g. engine=api_pool while model_name points at a Codex model) caused
        // runs to hit the API pool and fail while the UI looked like Codex.
        const engineForRun: AgentEngine | undefined =
          modelEntry?.engine != null
            ? (modelEntry.engine as AgentEngine)
            : (context.engine as AgentEngine | undefined);
        ...
        await thread.submit(
          ...
          },
          {
            ...
            context: {
              ...
              ...context,
              engine: engineForRun,
              runtime_profile: context.mode,
              thinking_enabled: context.mode !== "flash",
              // Keep Ultra focused on execution/delegation rather than todo-list planning.
              // This avoids long hidden-step planning loops for day-to-day coding tasks.
              is_plan_mode: context.mode === "pro",
              subagent_enabled: context.mode === "ultra" && context.model_name !== "glm-5.1",
              reasoning_effort:
    ...
    [thread, _handleOnStart, t.uploads.uploadingFiles, context, models, queryClient],
  );
