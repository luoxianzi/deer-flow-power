...
type InputEngine = "codex" | "claude" | "api_pool";

function isInputEngine(value: unknown): value is InputEngine {
  return value === "codex" || value === "claude" || value === "api_pool";
}
...
function getResolvedEngine(
  engine: InputEngine | undefined,
  modelEngine: unknown,
): InputEngine {
  if (isInputEngine(engine)) {
    return engine;
  }
  if (isInputEngine(modelEngine)) {
    return modelEngine;
  }
  return "codex";
}
...
export function InputBox({
  ...
  const selectedEngine = useMemo<InputEngine>(() => {
    const currentModel = models.find((m) => m.name === context.model_name);
    return getResolvedEngine(context.engine, currentModel?.engine);
  }, [context.engine, context.model_name, models]);

  const selectableModels = useMemo(() => {
    const filteredModels = models.filter((m) => m.engine === selectedEngine);
  ...
  useEffect(() => {
    ...
    const currentModel = selectableModels.find((m) => m.name === context.model_name);
    const fallbackModel = currentModel ?? selectableModels[0]!;
    const supportsThinking = fallbackModel.supports_thinking ?? false;
    const nextModelName = fallbackModel.name;
    const nextMode = getResolvedMode(context.mode, supportsThinking);

    if (
      context.engine === selectedEngine &&
  ...
  }, [context, models, onContextChange, selectableModels, selectedEngine]);
  ...
  }, [context.model_name, selectableModels]);

  const selectedModelSecondaryLine = useMemo(() => {
    if (!selectedModel) {
      return undefined;
    }
    if (selectedModel.credential_label) {
      return `${selectedModel.credential_label} · ${selectedModel.model}`;
    }
  ...
  const handleModelSelect = useCallback(
    (model_name: string) => {
      ...
      onContextChange?.({
        ...context,
        engine: getResolvedEngine(context.engine, model.engine),
        model_name,
  ...
  const handleEngineSelect = useCallback(
    (engine: InputEngine) => {
      const nextModels = models.filter((m) => m.engine === engine);
      const nextModel = nextModels[0] ?? models[0];
      if (!nextModel) {
        return;
      }
      ...
      onContextChange?.({
        ...
        mode: getResolvedMode(context.mode, nextModel.supports_thinking ?? false),
        reasoning_effort: context.reasoning_effort,
      });
    },
  ...
  const getEngineLabel = useCallback(
    (engine: InputEngine) => {
      switch (engine) {
        case "claude":
          return t.inputBox.engineClaude;
        case "api_pool":
          return t.inputBox.engineApiPool;
        case "codex":
        default:
          return t.inputBox.engineCodex;
      }
    },
    [t],
  );
  ...
  const getEngineDescription = useCallback(
    (engine: InputEngine) => {
      switch (engine) {
        case "claude":
          return t.inputBox.engineClaudeDescription;
        case "api_pool":
          return t.inputBox.engineApiPoolDescription;
        case "codex":
        default:
          return t.inputBox.engineCodexDescription;
      }
    },
    [t],
  );
          ...
          <PromptInputTools>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-80">
                ...
                <DropdownMenuSeparator />
                {(["codex", "claude", "api_pool"] as const).map((engine) => (
                  <DropdownMenuItem
                    key={engine}
                    className="items-start gap-3 py-3"
                    onSelect={() => handleEngineSelect(engine)}
                  >
                    ...
                    {selectedEngine === engine ? (
                      <CheckIcon className="mt-0.5 ml-auto size-4 shrink-0" />
                    ) : (
