...
- **模型路由更稳**：新增 Claude / Codex / API Pool 三类运行时统一路由，前端选择的模型优先于 agent 固定模型。
- **API Pool 更抗失败**：支持 OpenAI-compatible gateway、多 gateway、多 key 轮转；对鉴权/限流/并发满载/网关超时做分类恢复。
- **Claude OAuth 自动续期**：支持读取 Claude Code OAuth 凭证、过期刷新、Bearer 鉴权、prompt caching、Opus 4.7 adaptive thinking。
- **Agent 并发能力提升**：subagent 默认并发 `3 -> 6`，硬上限 `4 -> 8`，执行线程池 `3 -> 8`。
- **Sandbox 更适合落地**：增加 Docker power profile、2 个预热 sandbox replica、HTTP bridge 健康检查、连接失败自动恢复和凭证桥接。
- **详细优化说明**：见 [OPTIMIZATIONS.md](./OPTIMIZATIONS.md)。

## 如何利用 Agent 提升效率

这个版本把复杂研发任务拆成“主 Agent + 多个 subagent”的协作流程：主 Agent 负责需求理解、任务拆分、上下文整合和最终交付；subagent 分别负责代码阅读、实现修改、回归测试、文档整理、故障定位。因为模型、工具、sandbox 和文件系统都在同一套 harness 内，Agent 可以直接读写项目、运行测试、生成说明文档，并把结果回传给主流程。
...
> Power 优化版：本仓库在官方 DeerFlow 基础上加入了 Claude/Codex/API-pool 路由、更高 subagent 并发、sandbox 自动恢复和 Docker power profile。详细说明见 [OPTIMIZATIONS.md](./OPTIMIZATIONS.md)。
