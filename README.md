# DeerFlow Power：面向 Agent 协作的优化版

本仓库不是官方 DeerFlow 的原样镜像，而是基于 `bytedance/deer-flow` 做的本地增强版。目标是把 DeerFlow 从通用 super agent harness 优化成更适合真实研发、长任务、多模型切换和多 Agent 并行协作的工程平台。

## 相比官方 DeerFlow 的核心优化

- **模型路由更稳**：新增 Claude / Codex / API Pool 三类运行时统一路由，前端选择的模型优先于 agent 固定模型，避免“界面切了模型、实际仍用旧模型”。
- **API Pool 更抗失败**：支持 OpenAI-compatible gateway、多 gateway、多 key 轮转；对 `401/403/429` 自动换 key，对 `408/409/502/503/504/524` 做瞬时错误重试，`409` 并发满载默认至少重试 10 次。
- **Claude OAuth 自动续期**：支持读取 Claude Code OAuth 凭证、过期刷新、Bearer 鉴权、billing header、prompt caching、Opus 4.7 adaptive thinking 和 reasoning effort。
- **Agent 并发能力提升**：subagent 默认并发从 `3` 提到 `6`，硬上限从 `4` 提到 `8`，执行线程池从 `3` worker 提到 `8` worker，适合把代码分析、实现、测试、文档拆成多个并行子任务。
- **Sandbox 更适合工程落地**：增加 Docker power profile、2 个预热 sandbox replica、HTTP bridge 健康检查、连接失败自动恢复、本地 fallback、SSH/Git/Claude/Codex 凭证桥接。
- **可审计的公开说明**：详细改动、效率收益和测试结果见 [OPTIMIZATIONS.md](./OPTIMIZATIONS.md)。

## 如何利用 Agent 提升效率

这个版本把一个复杂研发任务拆成“主 Agent + 多个 subagent”的协作流程：主 Agent 负责需求理解、任务拆分、上下文整合和最终交付；subagent 可以分别负责代码阅读、实现修改、回归测试、文档整理、故障定位。因为模型、工具、sandbox 和文件系统都在同一套 harness 内，Agent 可以直接读写项目、运行测试、生成说明文档，并把结果回传给主流程。
...
> Power fork: this repository includes local optimizations for Claude/Codex/API-pool routing, higher subagent concurrency, sandbox recovery, and a Docker power profile. See [OPTIMIZATIONS.md](./OPTIMIZATIONS.md).
